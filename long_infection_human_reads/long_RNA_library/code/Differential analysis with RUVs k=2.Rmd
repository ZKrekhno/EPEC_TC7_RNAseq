...
data_path <- "long_infection_human_reads/long_RNA_library/data/"
res_path <- "long_infection_human_reads/long_RNA_library/results/final clean figures/"
```
...
``` {r dds, fig.width=7, fig.height=5}
...
normal_PCA <- DESeq2::plotPCA(rld2.nobatch, returnData = T, intgroup = "group") %>% 
  mutate(genotype = str_remove(group, "[:digit:]+")) %>% 
  mutate(genotype = fct_relevel(genotype, "UI", "WT", "N"))
#png(here::here("long_infection_human_reads/long_RNA_library/results/final clean figures/PCA plot of normalized counts legend bottom.png"), width = 7, height = 5, units = "in", res = 1080)
pca_plot <- ggplot(data = normal_PCA, aes(shape = genotype, color = genotype)) +
  scale_color_manual(labels = c("N" = expression(Delta*italic("escN")),
                                "WT" = "WT", "UI" = "Uninfected"),
  ...
  labs(shape = "Infection Status", color = "Infection Status",
       x = "PC1: 88% variance", y = "PC2: 6% variance") +
  scale_shape_manual(labels = c("N" = expression(Delta*italic("escN")),
                                "WT" = "WT", "UI" = "Uninfected"),
...
#Remove lncRNA from this heatmap
mRNA_index <- str_which(rownames(heat_mat), "(ENSG)|(MIR)|(-AS)", negate = T)
heat_mat <- heat_mat [mRNA_index,]
topVarGenes <- head(order(-rowVars(heat_mat)),100)
...
pheatmap(mat, annotation_col=df, 
         ...
         show_rownames = F, show_colnames = F, border_color = NA,
         color = color_scheme,
         filename = here::here("long_infection_human_reads/long_RNA_library/results/final clean figures/top 100 DE heatmap.png"),
         width = 8, height = 6)
```
...
Repeat EGSEA analysis with miRNA and TF sets
```{r egsea-msig-tf, eval=FALSE}
#Build gene set collection
gs.annots.tf <-  buildGMTIdx(geneIDs = rownames(cnt_obj_filt), species = "Human", 
                            gmt.file = here::here("long_infection_human_reads/long_RNA_library/data/c3.tft.v2023.1.Hs.entrez.gmt"))

gsa.tf <-  egsea.cnt(counts = cnt_obj_filt, group = group_obj, design = design_matrix,
                    contrasts = contrast_obj, gs.annots = gs.annots.tf, symbolsMap = geneids_smbl,
                    baseGSEAs = egsea.base(), sort.by = "p.adj", fdr.cutoff = 0.05, combineMethod = "wilkinson",
                    num.threads = 4, report = TRUE, 
                    report.dir = here::here("long_infection_human_reads/long_RNA_library/results/final clean figures/all comparisons MSIGDB TF"),
                    verbose = T, display.top = 35)
```
```{r egsea-msig-mir, eval=FALSE}
#Build gene set collection
gs.annots.mir <-  buildGMTIdx(geneIDs = rownames(cnt_obj_filt), species = "Human", 
                            gmt.file = here::here("long_infection_human_reads/long_RNA_library/data/c3.mir.v2023.1.Hs.entrez.gmt"))

gsa.mir <-  egsea.cnt(counts = cnt_obj_filt, group = group_obj, design = design_matrix,
                    contrasts = contrast_obj, gs.annots = gs.annots.mir, symbolsMap = geneids_smbl,
                    baseGSEAs = egsea.base(), sort.by = "p.adj", fdr.cutoff = 0.05, combineMethod = "wilkinson",
                    num.threads = 4, report = TRUE, 
                    report.dir = here::here("long_infection_human_reads/long_RNA_library/results/final clean figures/all comparisons MSIGDB mir"),
                    verbose = T, display.top = 35)
```
...
write.table(assay(rld2.nobatch), here::here("long_infection_human_reads/long_RNA_library/results/final clean figures/tables/rld_count_matrix_RUV_corrected.txt"),
            sep = "\t", row.names = T)
