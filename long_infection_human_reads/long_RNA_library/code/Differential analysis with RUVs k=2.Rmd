...
#Print the PCA plot out
pca_plot <- ggplot(data = normal_PCA, aes(x = PC1, y = PC2, shape = genotype, color = genotype)) +
...
df <- as.data.frame(colData(rld)) %>% 
  ...
  transmute(`Infection Status` = fct_recode(genotype, "Mutant" = "N",
                                            "WT" = "WT","Uninfected" = "UI")) 
#svg(here::here(str_c(res_path, "top 100 DE heatmap.svg")), width = 8, height = 6)
...
pheatmap(mat, annotation_col=df, 
         ...
         show_rownames = F, show_colnames = F, border_color = NA,
         color = color_scheme,
         filename = here::here(str_c(res_path, "top 100 DE heatmap.png")),
         width = 8, height = 6)
```
