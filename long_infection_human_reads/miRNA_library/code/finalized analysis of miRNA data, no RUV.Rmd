...
data_path <- "long_infection_human_reads/miRNA_library/data/"
res_path <- "long_infection_human_reads/miRNA_library/results/final clean figures/"
```
...
normal_PCA <- DESeq2::plotPCA(rld, returnData = T, intgroup = "group") %>% 
...
volcano_ggplot <- function(res_df, title = "", to_label = F, n_lab = 10) {
  ...
  if(to_label) {
    res_df_top <- res_df %>% 
      ...
      slice_head(n=n_lab) %>% 
      separate(gene, into = c("gene","origin_miRNA"), sep = "_")
    p_labelled <- p_empty + 
      geom_label_repel(data = res_df_top, aes(x = log2FoldChange, y = -log10(padj), label = gene),
                       size = 6, max.overlaps = Inf, color = "black", fill = "white", force = 100, force_pull = 1)
    return(p_labelled)
  } else {
    return(p_empty)
  }
...
```{r volcano-print, fig.width=8, fig.height=6}
...
#dev.off()
#map2(volcano_list_labelled, c("WTvsN","WTvsUI","dNvsUI"), 
 #    ~ggsave(filename = here::here(str_c("long_infection_human_reads/miRNA_library/results/final clean figures/Volcano plots",.y,".svg")), plot = .x, width = 8, height = 6))
#Print labelled plots into pngs
#walk2(volcano_list_labelled, c("WTvsN","WTvsUI","NvsUI"),
 #     ~ggsave(here::here(str_c("long_infection_human_reads/miRNA_library/results/final clean figures/Volcano plots ", .y,".png")), 
  #            plot = .x, width = 8, height = 6, dpi = 1080))
