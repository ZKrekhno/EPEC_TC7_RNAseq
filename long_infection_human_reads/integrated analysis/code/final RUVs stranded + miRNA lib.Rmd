...
```{r multimir-prep}
mi_res <- read.table(here::here("mirna_normalized results.txt"), sep = "\t", header = T)
...
#pdf(here::here("final RUV figures/Venn diagrams of shared differentially expressed miRNA.pdf"), width = 12, height = 10)
...
ggplot(de_mirna) + 
  geom_point(aes(x=LFC,y=-log10(padj), color = LFC_comparison)) + 
  geom_hline(aes(yintercept=1), linetype = "longdash", lwd = 1.2, color = "darkgrey") +
  geom_vline(aes(xintercept=0.6), linetype = "longdash", lwd = 1.2, color = "darkgrey")+
  geom_vline(aes(xintercept=-0.6), linetype = "longdash", lwd = 1.2, color = "darkgrey")+
  facet_wrap(~fct_reorder(comb_name,-log10(padj)), scales = "free_y") + 
  theme_bw(base_size = 18)
ggsave(here::here("final RUV figures/Volcano plots of all DE miRNA.png"), width = 12, height = 10)
```
...
Next step is to prepare DE mRNA from WTvsN condition and perform multimiR analysis

```{r multimir-prep}
#Select just the upregulated and downregulated mRNA
m_res <- read.table(here::here("RUV_normalized results.txt"), sep = "\t", header = T) 
m_filt_up <- m_res %>% 
  filter(padj.WTvsN < 0.1 & log2FoldChange.WTvsN > 0) 
...
final_int <- rbind(inter_tbl_mi_up, inter_tbl_mi_down) %>% 
  mutate(symbol = case_when(
    ...
    TRUE ~ target_symbol))
#write.table(final_int, here::here("multimir miRNA-RUVsmRNA WTvsN validated interactions.txt"), 
...
c2 <- read.gmt(here::here("c2.cp.kegg.v7.5.1.symbols.gmt"))
```
...
#write.table(top_genes_symbol, here::here("attribute table for miRNA-RUVs mRNA.txt"), sep = "\t", row.names = F, quote = F)
...
top_apop_mirna <- final_int %>% 
  ...
  unique() 
#write.table(top_apop_mirna, here::here("final RUV figures/miRNA with apoptotic associations.txt"))
```
...
#pdf(here::here("final RUV figures/clusterprofiler WTvsN results ora analysis with defined background list.pdf"),
#width = 8, height = 8)
for (i in c(1,3)) {
barplot(ora [[i]], showCategory = 20, font.size = 14) %>% print()
dotplot(ora [[i]], showCategory = 20, font.size = 14) %>% print()
cnetplot(ora [[i]], foldChange=gl, font.size = 14) %>% print()
heatplot(ora [[i]], foldChange=gl, showCategory=5) %>% print()
}
#dev.off()
#Now repeat with gsea analysis
#Perform ora analysis with clusterprofiler
gsea.wtn <- vector(mode = "list",length = 3)
for (i in 1:length(gsea.wtn)) {
  gsea.wtn [[i]] <- GSEA(gl, TERM2GENE = sets [[i]], pvalueCutoff = 1)
}
i=1
#pdf(here::here("final RUV figures/clusterprofiler WTvsN results gsea analysis.pdf"),
 #width = 8, height = 8)
for (i in c(3)) {
dotplot(gsea.wtn [[i]], showCategory = 20, font.size = 14) %>% print()
cnetplot(gsea.wtn [[i]], foldChange=gl) %>% print()
heatplot(gsea.wtn [[i]], foldChange=gl, showCategory=20) %>% print()
heatplot(gsea.wtn [[i]], foldChange=gl, showCategory=5) %>% print()
heatplot(gsea.wtn [[i]], foldChange=gl, showCategory=10) %>% print()
}
#dev.off()
```
...
#write.table(inter_tbl, here::here("multimir miRNA-RUVsmRNA WTvsN validated interactions combined search.txt"), 
 #          sep = "\t", row.names = F, quote = F)
#Now we can make a table with attributes (gene species, and expression up, or down)
top_mrna <-  inter_tbl %>% 
  ungroup() %>% 
...
#write.table(top_genes_expr, here::here("attribute table for combined miRNA-RUVs mRNA.txt"), sep = "\t", row.names = #F, quote = F)
```
...
#pdf(here::here("final RUV figures/clusterprofiler WTvsN combined search ora analysis with defined background list.pdf"),
#width = 8, height = 5)
for (i in 1) {
barplot(ora [[i]], showCategory = 20, font.size = 14) %>% print()
dotplot(ora [[i]], showCategory = 20, font.size = 14) %>% print()
...
i=1
#pdf(here::here("final RUV figures/clusterprofiler WTvsN combined search gsea analysis.pdf"),
 #width = 8, height = 5)
for (i in c(1,3)) {
dotplot(gsea.wtn [[i]], showCategory = 20, font.size = 14) %>% print()
cnetplot(gsea.wtn [[i]], foldChange=gl) %>% print()
