...
gsets_h_add_missing <- gsets_h_add_missing %>% 
  mutate(Strain_label = factor(Strain, levels = c("Wild-type EPEC","T3SS-deficient EPEC"),
                               ordered = T, 
                               labels = c(expression(bold("WT")),
                                          expression(bold(Delta)*bolditalic("escN"))
                                          )))
...
ggplot(gsets_h_add_missing, aes(x = factor(Time), y = GeneSet)) +
  ...
  scale_fill_manual(values = c("-1" = carto_pal(2, diverg_pal_choice) [1],
                    ...
                    na.value = "grey80") +
  labs(x = "Time of Infection (min)", y = element_blank(), fill = "Regulation Direction") +
  facet_wrap(~Strain_label, ncol = 2, labeller = label_parsed) +
