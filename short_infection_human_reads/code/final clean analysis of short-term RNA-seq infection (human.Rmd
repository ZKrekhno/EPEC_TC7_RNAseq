...
```{r readorigstar}
counts <- read.table("STAR_reads_all.txT",header = F, sep = "\t", stringsAsFactors = F)
...
counts_simple <- counts [,seq(from=4,to=88,by=4)]
colnames(counts_simple) <- c("4UIN", "4UIWT", "490WT", "490N", "4180N", "4180WT", "5UIN", "5UIWT", "545WT", "545N", "590WT", "590N", "5180N", "5180WT", "6UIN", "6UIWT", "645WT", "645N", "690WT", "690N", "6180N", "6180WT")
counts_simple <- counts_simple %>% 
  dplyr::select(-contains("45"), -`6UIN`)
...
#write.table(counts_simple,file="only_reverse_strand_counts_cleaned.txt",sep = "\t", row.names = F)
```
...
rel_count_plot <-ggplot(rel_map, aes(x=mapping, y=numbers, fill = Gene_ID)) +
  ...
  guides(fill = guide_legend(title = "Mapping Status"))
#pdf(file = here::here("final clean figures/mapping stats.pdf"), width = 8, height = 6.6)
raw_count_plot
rel_count_plot
#dev.off()
```
...
#pdf(file = here::here("final clean figures/mapping stats by gene type.pdf"), width = 8, height = 6.6)
rel_type_plot
#dev.off()
...
#Plot the counts 
#pdf(here::here("final clean figures/Normalization effect on the library.pdf"), width = 8, height = 6.6)
...
#png(here::here("final clean figures/PCA plot of normalized counts.png"), width = 8, height = 6.6, units = "in", res = 720)
ggplot(data = normal_PCA, aes(shape = pData$genotype)) +
  scale_color_manual(values = c("coral", "darkcyan","darkblue")) +
  geom_point(mapping = aes(x = PC1, y = PC2, color = as.factor(timepoint)), size = 4) +
  labs(title = "EDAseq normalized DESeq2 PCA plot \nnot including the outlier or 45min time") +
  guides(color = guide_legend(title = "Time of infection (min)"),
         shape = guide_legend(title = "Infection status")) +
  scale_shape_manual(labels = c("N" = paste0(greeks("Delta"),"EscN EPEC"), "WT" = "Wild-type EPEC"),
...
#write.table(time_res, here::here("final clean figures/EDAseq_normalized results comparing by time WT infection.txt"),
 #           sep = "\t", row.names = F)
...
#dev.off()
#In case want to show PC coordinates with respect samples with boxplots
pc_coord <- p$rotated %>% 
  as_tibble(rownames = "sample") %>% 
  left_join(as_tibble(pData,rownames = "sample")) %>% 
  pivot_longer(contains("PC"),names_to = "PC",values_to = "coord") 
ggplot(pc_coord) + geom_boxplot(aes(x = as.character(timepoint), y = coord)) +
