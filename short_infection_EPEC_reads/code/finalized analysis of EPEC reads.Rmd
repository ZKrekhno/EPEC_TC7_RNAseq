...
```{r readorigstar}
counts <- read.table("STAR_EPEC_reads_all.txT",header = F, sep = "\t", stringsAsFactors = F)
...
ggsave(here::here("final EPEC figures/raw counts for escN.png"),width = 6, height = 5, dpi = 1080)
```
...
rel_count_plot <-ggplot(rel_map, aes(x=mapping, y=numbers, fill = Gene_ID)) +
  ...
  guides(fill = guide_legend(title = "Mapping Status"))
#pdf(file = here::here("final EPEC figures/mapping stats.pdf"), width = 8, height = 6.3)
raw_count_plot
rel_count_plot
#dev.off()
```
