...
gl_df_list <- list(res45lfc,res90lfc,res180lfc, res45vsn,res90vsn, res180vsn) %>% 
...
pheatmap(mat, annotation_col=df, 
         ...
         show_rownames = T, show_colnames = F, border_color = NA, fontsize_row = 3.5,
         color = color_scheme)
```
...
mito_ids <- time_res_annot %>% 
  filter(true_name %in% c("E2348C_3942","espH","espF","espZ")) %>% 
...
ggplot(mito_counts, aes(x=timepoint,y=count, color = true_name)) +
...
#ggsave(here::here(str_c(res_path,"Mitochondrial effector counts over time.png")), width = 7, height = 5, dpi = 1080)
```
