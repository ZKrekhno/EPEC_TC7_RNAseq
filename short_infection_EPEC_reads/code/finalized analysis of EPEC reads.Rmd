...
pca_plot <- ggplot(data = normal_PCA, aes(shape = genotype)) +
  ...
  labs(x = "PC1: 40% variance",
       y = "PC2: 14% variance") +
  guides(color = guide_legend(title = "Time of Infection (min)", order = 1),
         shape = guide_legend(title = "Strain", order = 2)) +
...
#write.table(time_resvsn_annot, here::here(str_c(res_path, "tables/EDAseq_normalized results comparing  WT and EscN infection.csv")),
 #          sep = ",", row.names = F)
```
Interestingly, bfp genes are down-regulated during infection when T3SS is present, let's plot that
```{r bfp-plot, fig.width=7, fig.height=5}
#Filter results to bfp and per
bfp_ids <- time_resvsn_annot %>% 
  filter(str_detect(true_name, "bfp|per")) 
bfp_counts <- set_names(bfp_ids$geneid, bfp_ids$true_name) %>% 
  map(~plotCounts(dds, gene = .x, returnData = T, intgroup = "timepoint")) %>% 
  imap(~mutate(.x, true_name = .y)) %>% 
...
#Calculate z-score
bfp_z <- bfp_counts %>% 
  select(sample, true_name, count) %>%  
  pivot_wider(names_from = "sample", values_from = "count") %>% 
  column_to_rownames("true_name") %>% 
  as.matrix() 
bfp_z_scaled <- t(apply(bfp_z, 1, scale))
colnames(bfp_z_scaled) <- colnames(bfp_z)
#Set the breaks to be able to center the heatmap colors around 0
mycolors <- rcartocolor::carto_pal(name = diverg_pal_choice)
pal_length <- 255
mybreaks <-  c(seq(min(bfp_z_scaled), 0, length.out=ceiling(pal_length/2) + 1), 
                    seq(max(bfp_z_scaled)/pal_length, max(bfp_z_scaled), length.out=floor(pal_length/2)))
...
df <- as.data.frame(colData(rld)) %>%  
  mutate(`Strain` = fct_recode(genotype, "Mutant" = "N",
                                            "WT" = "WT"), .keep = 'unused') %>% 
...
sample_order_n <- as_tibble(colData(rld), rownames = "sample") %>% 
  ...
  pull(sample)
#Create the ordering and annotation of the bfp operon
bfp_order <- c(str_c("bfp",c("A","G","B","C","U","D","E","F","P","H","I","J","K","L","M")),
               str_c("per",c("A","B","C")))
df_row <- bfp_counts %>% 
  select(true_name) %>% 
  distinct() %>% 
  mutate(Locus = if_else(str_detect(true_name,"bfp"), "bfp", "per")) %>% 
  column_to_rownames(var = "true_name")
#svg(here::here(str_c(res_path,"bfp operon differences.svg")), width = 7, height = 5)
pheatmap(bfp_z_scaled [,c(sample_order_n, sample_order_wt)], 
         annotation_col=df, annotation_row = df_row, annotation_names_row = F,
         annotation_colors = list("Strain" = c("WT" =carto_pal(12,discrete_pal_choice) [2],
                                  ...
                                  "Locus" = c("bfp" = carto_pal(12,"Safe") [1],
                                              "per" = carto_pal(12,"Safe") [2])),
         fontsize = 14, treeheight_row = 0, treeheight_col = 0, breaks = mybreaks,
         show_rownames = T, show_colnames = F, border_color = NA, fontsize_row = 14,
         gaps_row = 14, legend = F, annotation_names_col = F, 
         color = color_scheme, cluster_rows = F, cluster_cols = F)
#dev.off()
```


```{r bfp-plot-print,eval=FALSE,fig.width=7, fig.height=5}
pheatmap(bfp_z_scaled [,c(sample_order_n, sample_order_wt)], 
         annotation_col=df, annotation_row = df_row, annotation_names_row = F,
         annotation_colors = list("Strain" = c("WT" =carto_pal(12,discrete_pal_choice) [2],
                                                         "Mutant" = carto_pal(12,discrete_pal_choice) [1]),
         ...
         color = color_scheme, cluster_rows = F, cluster_cols = F,
         width = 7, height = 5, 
         filename = here::here(str_c(res_path,"bfp operon differences.png")))
```
...
df <- as.data.frame(colData(rld)) %>%  
  ...
  mutate(`Time of Infection (min)` = fct_recode(as.factor(timepoint), "0" = "0", "45" = "45",
                                "90" = "90","180" = "180")) %>% 
  select(-group, -timepoint)
#svg(here::here(str_c(res_path,"top 100 DE genes WT180.svg")), width = 8, height = 5)
pheatmap(mat [,c(sample_order_n, sample_order_wt)], annotation_col=df, 
         annotation_colors = list("Strain" = c("WT" =carto_pal(12,discrete_pal_choice) [2],
                                  ...
                                  "Time of Infection (min)" = c("0" = chosen_cont_colors [1],
                                                  ...
                                                  "90" = chosen_cont_colors [3],
                                                  "180" = chosen_cont_colors [4])),
         fontsize = 11, treeheight_row = 0, treeheight_col = 0, breaks = mybreaks, cluster_cols = F,
